...
from services.nba_documentation_service import (
    get_report2_documentation,
    get_report3_documentation,
    get_report4_documentation,
    get_report5_documentation,
    get_report6_documentation,
    get_report7_documentation,
    get_report8_documentation,
    get_report9_documentation,
)
...
from services.nba_report6_service import get_direct_co_po_contribution
from services.nba_report7_excel import generate_report7_excel
from services.nba_report7_service import get_indirect_co_po_contribution
...
def nba_report1(course_id: int):
    ...
    return StreamingResponse(
        ...
        headers={
            "Content-Disposition": 'attachment; filename="Student_Marks_Register.xlsx"'
        },
    )


@router.get("/nba-report2", response_class=HTMLResponse)
def nba_report2(request: Request, course_id: int):
    course = get_course_details(course_id)
    target_score = 10

    attainment = get_course_co_summary(course_id, target_score)
    ...
    documentation = get_report2_documentation()
    return templates.TemplateResponse(
        request=request,
...
def nba_report2_excel(course_id: int):
    wb = generate_direct_co_attainment_excel(course_id, target_score=10)
    ...
    return StreamingResponse(
        ...
        media_type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        headers={
            "Content-Disposition": 'attachment; filename="Direct_CO_Attainment.xlsx"'
        },
    )


@router.get("/nba-report3", response_class=HTMLResponse)
def nba_report3(request: Request, course_id: int):
    course = get_course_details(course_id)
    ...
    documentation = get_report3_documentation()
    return templates.TemplateResponse(
        request=request,
...
def nba_report3_excel(course_id: int):
    ...
    return StreamingResponse(
        ...
        media_type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        headers={
            "Content-Disposition": 'attachment; filename="Indirect_CO_Attainment.xlsx"'
        },
    )


@router.get("/nba-report4", response_class=HTMLResponse)
def nba_report4(request: Request, course_id: int):
    course = get_course_details(course_id)
    target_score = 10
    ...
    documentation = get_report4_documentation()
    return templates.TemplateResponse(
        request=request,
...
def nba_report4_excel(course_id: int):
    wb = generate_final_co_attainment_excel(course_id, target_score=10)
    ...
    return StreamingResponse(
        ...
        media_type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        headers={
            "Content-Disposition": 'attachment; filename="Final_CO_Attainment.xlsx"'
        },
    )


@router.get("/nba-report5", response_class=HTMLResponse)
def nba_report5(request: Request, course_id: int):
    course = get_course_details(course_id)
    mappings = get_course_mappings(course_id)
    documentation = get_report5_documentation()
...
def nba_report5_excel(course_id: int):
    ...
    return StreamingResponse(
        ...
        media_type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        headers={
            "Content-Disposition": 'attachment; filename="CO_PO_Mapping.xlsx"'
        },
    )


@router.get("/nba-report6", response_class=HTMLResponse)
def nba_report6(request: Request, course_id: int):
    course = get_course_details(course_id)
    contribution = get_direct_co_po_contribution(course_id)
    documentation = get_report6_documentation()
...
def nba_report6_excel(course_id: int):
    ...
    return StreamingResponse(
        ...
        media_type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        headers={
            "Content-Disposition": 'attachment; filename="Direct_CO_PO_Contribution.xlsx"'
        },
    )


@router.get("/nba-report7", response_class=HTMLResponse)
def nba_report7(request: Request, course_id: int):
    course = get_course_details(course_id)
    contribution = get_indirect_co_po_contribution(course_id)
    documentation = get_report7_documentation()
...
def nba_report7_excel(course_id: int):
    ...
    return StreamingResponse(
        ...
        media_type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        headers={
            "Content-Disposition": 'attachment; filename="Indirect_CO_PO_Contribution.xlsx"'
        },
    )


@router.get("/nba-report8", response_class=HTMLResponse)
def nba_report8(request: Request, course_id: int):
    course = get_course_details(course_id)
    contribution = get_final_co_po_contribution(
        course_id, target_score=10
    )
    documentation = get_report8_documentation()
...
def nba_report8_excel(course_id: int):
    wb = generate_report8_excel(course_id, target_score=10)
    ...
    return StreamingResponse(
        ...
        media_type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        headers={
            "Content-Disposition": 'attachment; filename="Final_CO_PO_Contribution.xlsx"'
        },
    )


@router.get("/nba-report9", response_class=HTMLResponse)
def nba_report9(request: Request, course_id: int):
    course = get_course_details(course_id)
    target_score = 10
    ...
    contribution = get_final_co_po_contribution(
        course_id, target_score
    )
    documentation = get_report9_documentation()
    return templates.TemplateResponse(
        request=request,
...
def nba_report9_excel(course_id: int):
    wb = generate_report9_excel(course_id, target_score=10)
    ...
    return StreamingResponse(
        ...
        media_type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        headers={
            "Content-Disposition": 'attachment; filename="Final_PO_PSO_Attainment_Summary.xlsx"'
        },
    )