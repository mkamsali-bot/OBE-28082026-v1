...
def generate_report6_excel(course_id, target_score=10):
    ...
    for item in contribution:
        ...
        # Direct CO Assessment
        if item.get("co_attainment") is not None:
            ws.cell(
                row=row,
                column=2
        ...
        # CO Attainment Level
        if item.get("co_level") is not None:
            ws.cell(
                row=row,
                column=3
            ).value = item["co_level"]
